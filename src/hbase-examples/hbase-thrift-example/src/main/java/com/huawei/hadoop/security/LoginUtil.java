...
public class LoginUtil {
    ...
    private static boolean checkNeedLogin(String principal) throws IOException {
        ...
        if ((currentUser != null) && (currentUser.hasKerberosCredentials())) {
            if (checkCurrentUserCorrect(principal)) {
                LOG.info("current user is {} has logined.", currentUser);
    ...
    public static void setJaasFile(String principal, String keytabPath) throws IOException {
        ...
        String keyTabFilePath = keytabPath.replace("\\", "\\\\");
        // 删除jaas文件
        deleteJaasFile(jaasPath);
        writeJaasFile(jaasPath, principal, keyTabFilePath);
    ...
    public static void setJaasConf(String loginContextName, String principal, String keytabFile) throws IOException {
        checkContext(loginContextName, principal, keytabFile);
        ...
        for (int i = 0; i < entrys.length; i++) {
            ...
            if (IS_IBM_JDK) {
                ...
                if (entrys[i].getOptions().get("keyTab").equals(userKeytabFile.getCanonicalPath())) {
                    checkKeytab = true;
                }
            }
        }
        ...
        if (!checkPrincipal) {
            LOG.error("AppConfigurationEntry named {} does not have principal value of {}.",
        ...
        if (!checkKeytab) {
            LOG.error("AppConfigurationEntry named {} does not have keyTab value of {}.",
     ...
     *
     * @param zkServerPrincipal zookeeper server principal
     * @throws IOException
     */
     ...
     * setZookeeperServerPrincipal
     *
     * @param zkServerPrincipalKey zookeeper server principal key
     * @param zkServerPrincipal zookeeper server principal
     * @throws IOException
     * @deprecated new method with the same name already exists.
     */
    @Deprecated
    ...
    private static boolean checkCurrentUserCorrect(String principal) throws IOException {
        ...
        if ((defaultRealm != null) && (defaultRealm.length() > 0)) {
            ...
            if (!principal.endsWith(realm.toString())) {
                ...
                resPrincipal = principalWithRealm.toString();
            }
        }

        return resPrincipal.equals(ugi.getUserName());
    }
    ...
    private static class JaasConfiguration extends javax.security.auth.login.Configuration {
        ...
        static {
            String jaasEnvDebug = System.getenv("HBASE_JAAS_DEBUG");
            if ("true".equalsIgnoreCase(jaasEnvDebug)) {
        ...
        private JaasConfiguration(String loginContextName, String thePrincipal, String keytabFile, boolean useTicketCache)
                throws IOException {
            try {
                this.loginConfig = javax.security.auth.login.Configuration.getConfiguration();
            ...
            initKerberosOption();
            LOG.info("JaasConfiguration loginContextName={} thePrincipal={} useTicketCache={} keytabFile={}",
                loginContextName, thePrincipal, useTicketCache, keytabFile);
        }
        ...
        private void initKerberosOption() throws IOException {
            ...
            KEYTAB_KERBEROS_OPTIONS.put("principal", thePrincipal);
        }
        ...
        public AppConfigurationEntry[] getAppConfigurationEntry(String appName) {
            ...
            if (loginConfig != null) {
                return loginConfig.getAppConfigurationEntry(appName);
            }
            return null;
        }
    }
