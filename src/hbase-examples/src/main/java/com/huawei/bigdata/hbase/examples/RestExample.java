...
public class RestExample extends HBaseExample{

    private final static Log LOG = LogFactory.getLog(RestExample.class.getName());

    private static Configuration conf = null;
    private final static String REST_TABLE_NAME = "hbase_rest_table";
    ...
    private void createdAndAlterNamespacesByJSON() {
        ...
        ObjectMapper jsonMapper = new JacksonJaxbJsonProvider().locateMapper(NamespacesInstanceModel.class, MediaType.APPLICATION_JSON_TYPE);
    ...
    private void testRestCreateTable() {
        ...
        LOG.info("Entering testCreateRestTable: tableName: " + tableName);

        /**
         * Because RemoteAdmin#createTable(HTableDescriptor desc), we can not use TableDescriptorBuilder instead.
         */
    ...
    public void testRestPut() {
        ...
        try {
            ...
            rTable = new RemoteHTable(client, REST_TABLE_NAME);
    ...
    public void testFilterList() {
        ...
        try {
            ...
            list.addFilter(new SingleColumnValueFilter(Bytes.toBytes("info"), Bytes.toBytes("age"),
                    CompareOperator.GREATER_OR_EQUAL, Bytes.toBytes(new Long(20))));
            // Obtain data with age of less than or equal to 29.
            list.addFilter(new SingleColumnValueFilter(Bytes.toBytes("info"), Bytes.toBytes("age"),
                    CompareOperator.LESS_OR_EQUAL, Bytes.toBytes(new Long(29))));
    ...
    public void dropRestTable() {
        ...
        try {
            initClient();
            restAdmin = new RemoteAdmin(client, conf);
            if (restAdmin.isTableAvailable(REST_TABLE_NAME)) {
                // Delete table.
                restAdmin.deleteTable(REST_TABLE_NAME);
            }
