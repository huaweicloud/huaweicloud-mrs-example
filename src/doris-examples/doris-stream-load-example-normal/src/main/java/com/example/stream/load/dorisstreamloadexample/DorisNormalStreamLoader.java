 ...
 * @since 2024-04-20
 */
public class DorisNormalStreamLoader {
    private static final Logger logger = LogManager.getLogger(DorisNormalStreamLoader.class);

    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();
    // FE IP Address
    private static String HOST = "";
    // FE port 普通模式使用 http_port
    private static long PORT = 29980;
    ...
    public static void initConf() throws IOException {
        ...
        InputStream in = DorisNormalStreamLoader.class.getClassLoader().getResourceAsStream("conf.properties");
        // 使用properties对象加载输入流
        properties.load(in);
        //获取key对应的value值
    ...
    public static HttpPut getHttpPut(String url, String label) {
    ...
    public static void testHttpClient() throws Exception {
        ...
        String url = String.format("http://%s:%s/api/%s/%s/_stream_load",
                HOST, PORT, DATABASE, TABLE);
        String label = "label_stream_load_" + UUID.randomUUID().toString();
        HttpPut feHttpPut = getHttpPut(url, label);
        ...
        HttpPut beHttpPut = getHttpPut(beLocation, label);
        // data
