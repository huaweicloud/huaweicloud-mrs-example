...
# ./bin/spark-submit SparkRExample/src/main/r/RSparkSQLExample.R
...
df <- read.json("hdfs://hacluster/example/people.json")
...
df <- read.df("hdfs://hacluster/example/users.parquet")
write.df(select(df, "name", "favorite_color"), "namesAndFavColors.parquet")
# $example off:generic_load_save_functions$


# $example on:manual_load_options$
df <- read.df("hdfs://hacluster/example/people.json", "json")
namesAndAges <- select(df, "name", "age")
write.df(namesAndAges, "namesAndAges.parquet", "parquet","overwrite")
# $example off:manual_load_options$


# $example on:manual_load_options_csv$
df <- read.df("hdfs://hacluster/example/people.csv", "csv",header=TRUE, sep = ";")
namesAndAges <- select(df, "name", "age")
# $example off:manual_load_options_csv$


# $example on:direct_sql$
df <- sql("SELECT * FROM parquet.`hdfs://hacluster/example/users.parquet`")
...
write.parquet(df, "people.parquet","overwrite")
...
path <- "hdfs://hacluster/example/people.json"
...
sql("LOAD DATA INPATH 'hdfs://hacluster/example/kv1.txt' INTO TABLE src")
