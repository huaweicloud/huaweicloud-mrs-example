...
public class SessionExample {
  ...
  static {
    ...
    nodeUrls.add(String.format("%s:%s", HOST_3, PORT));
  }
  ...
  private static void rawDataQuery() throws IoTDBConnectionException, StatementExecutionException {
    ...
    SessionDataSet dataSet = session.executeRawDataQuery(paths, startTime, endTime, queryTimeOut);
    System.out.println(dataSet.getColumnNames());
  ...
  private static void setTimeout() throws StatementExecutionException {
    Session tempSession = new Session(HOST_1, 6667, "root", "root", 10000, 20000);
