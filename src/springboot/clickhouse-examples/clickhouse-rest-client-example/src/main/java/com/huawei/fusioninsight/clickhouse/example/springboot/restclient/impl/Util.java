...
public class Util {
    ...
    static ClickHouseProperties clickHouseProperties = new ClickHouseProperties();
    ...
    private static void initProperties(ClickHouseProperties clickHouseProperties) {
        ...
        if (ClickHouseFunc.isSec && ClickHouseFunc.isMachineUser) {
            ...
            clickHouseProperties.setMachineUserKeytabPath(System.getProperty("user.dir") + "clickhouse-rest-client-example" + File.separator + File.separator + "conf" + File.separator + "user.keytab");
    ...
    private static boolean execSqlOnServer(String sql, String server, ClickHouseProperties clickHouseProperties, List<List<String>> resultArrayList) throws SQLException {
        ClickHouseDataSource clickHouseDataSource = new ClickHouseDataSource(JDBC_PREFIX + server, clickHouseProperties);
        ResultSet resultSet = null;
        try (Connection connection = clickHouseDataSource.getConnection(clickHouseProperties.getUser(), clickHouseProperties.getPassword()); PreparedStatement statement = connection.prepareStatement(sql);) {
    ...
    public static void insertData(String databaseName, String tableName, int batchNum, int batchRows) throws Exception {
        ...
        for (int tries = 1; tries <= serverList.size(); tries++) {
            ...
            try (Connection connection = clickHouseDataSource.getConnection(clickHouseProperties.getUser(), clickHouseProperties.getPassword());
