...
enter_bench StreamingRepartitionPrepare ${workload_config} ${current_dir}
show_bannar start

DATA_FILE1=${STREAMING_DATA1_DIR}/uservisits
DATA_FILE2=${STREAMING_DATA2_SAMPLE_DIR}

JVM_OPTS="-Xmx1024M -server -XX:+UseCompressedOops -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:+CMSClassUnloadingEnabled -XX:+CMSScavengeBeforeRemark -XX:+DisableExplicitGC -Djava.awt.headless=true -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false -Dkafka.logs.dir=bin/../logs -cp ${HADOOP_CONF_DIR}:${DATATOOLS}"
